...
| `ms-calendar.yaml` | `graph.microsoft.com` + `login.microsoftonline.com` (adds DELETE vs the built-in `outlook` preset) | `runmod-finn-live.sh` |
| `notion.yaml` | `api.notion.com` GET/POST/PATCH (no DELETE exists in the API) | `runmod-notion-live.sh` |
