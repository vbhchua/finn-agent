...
<img src="https://img.shields.io/badge/inference-NVIDIA%20NIM%20%C2%B7%20Nemotron-76B900" alt="NVIDIA NIM · Nemotron">
...
> **This is the stack finn actually runs**: OpenClaw **2026.6.10**, web search + full-page fetch,
> Telegram control, the calendar + Notion MCP servers, and the radar loops. All five steps are
> standard — run them in order. Secrets live in a gitignored **`.env`** (template:
> `.env.sample`); every `./…` script is idempotent (re-run after any rebuild). One-time
> prerequisites (the 2026.6.10 base build + vendored NemoClaw patch, the Entra app, the Notion
> integration) and troubleshooting live in **[SETUP.md](SETUP.md)**.

```bash
set -a; . ./.env; set +a                    # load all keys (.env.sample lists the 8 needed)
...
the sandboxed-agent scaffold (OpenShell runtime, deny-by-default egress, NIM/Nemotron inference)
that everything here builds on. This repo adds the DevRel research stack: the Firecrawl fetch
layer, the calendar + Notion MCP servers, and the conference-radar loops.
